...
public static class RoomBuilder
{
    ...
    //   '#'  厚牆：頂面 6px + 牆面 25px ≈ 兩格。東西掛得上去，代價是佔版面。
    //   '='  細牆：【只畫那條 6px 的頂面線】，沒有牆面——LimeZu 官方 Gym_2 的四邊就是這樣，
    //        量出來橫牆 N+WWWW+N＝6px、直牆 N+WWWWW+N＝7px，線後面直接是地板。
    //        外牆用細的：沒有東西要掛在上面，而厚牆一圈把大廳框得很悶。門那道牆維持厚的，
    //        它的牆面正是掛畫掛得住的原因。
    //
    ...
    static readonly string[] West =
    {
        "=============",
        "=============",   // 北外牆（細）
        "=TTTT.#.TTT.=",   // 左：櫃檯（訪客站 x5）｜ 右：會議室（長桌 x8-10）
        "=TTTT.#.TTT.=",   //   兩間之間是【垂直】牆，一欄就夠——垂直牆本來就是一欄寬。
        "=TTTT.#.TTT.=",   //   那道維持 '#'：它是室內隔間，兩側都看得到牆面才有厚度。
        "=............",   // ← 門內走廊，(12,5) 接辦公區
        ...
        "=TT.TT..TT.T=",   // 門外大廳（掛畫掛在上面那道牆的牆面上）：圓椅 x1-2、影印機／飲水機 x4-5、
                           //   書架 x8-9、販賣機 x11
        "=TT.........=",   //   藍色候客椅 x1-2
        "=...........=",
        ...
        "=============",   // 南外牆（細）。仍是兩列，但只有【貼著大廳那一列】會畫出那條線，
        "=============",   //   外面那列什麼都不畫——它在建築物外面，本來就不該有東西。
    };
