...
public static class RoomBuilder
{
    ...
    //   '#'  厚牆：頂面 6px + 牆面 25px + 陰影。西區已經【不再使用】——留著是因為
    //        compose_room 還認得它，哪天有東西非掛在牆面上不可時可以單獨改回某一段。
    //   '='  立體牆面：整段填滿白色頂面，每一個【面向房間】的邊收一條深藍外框。
    //        結構取自 LimeZu 官方 Gym_2：它的直牆剖面就是 N+WWWWW+N＝外框+填滿+外框。
    //        牆連成一個封閉的環，轉角由橫向那段收頭（見 compose_room 的細牆段）。
    //        代價：沒有牆面就掛不了東西，原本那幅彩色掛畫已經拿掉。
    //
    ...
    static readonly string[] West =
    {
        ...
        "=============",   // 南外牆。兩列都填滿，外框只收在【貼著大廳】那一側——
        "=============",   //   另一側是建築物外面，那裡沒有房間要跟它分界。
    };
