...
public static class RoomBuilder
{
    ...
    //
    //     0123456789012
    //  2  #TTTT.#.TT..#   櫃檯（左）｜ 會議室（右），中間單格牆隔開
    //  5  #............   ← 門內走廊，直接接辦公區
    //  7  ######DD#####   ← 公司大門（自動門 2 格寬）
    //  8  #...........#   門外大廳；掛畫掛在第 7 列那道牆上，【不佔】地板格
    // 11  #.TT.TT#TTTT#   自助角：機器【排一排】靠牆；等候椅在大廳這側

    static readonly string[] West =
    {
        "#############",
        "#############",   // 北外牆 2 格（牆的美術本來就是「頂面＋牆面」兩格）
        "#TTTT.#.TT..#",   // 左：櫃檯（辦公桌組佔滿，訪客站 x5）｜ 右：會議室（長桌 x8-9）
        "#TTTT.#.TT..#",
        "#TTTT.#.TT..#",
        "#............",   // ← 門內走廊，(12,5) 接辦公區
        "#...........#",
        "######DD#####",   // ← 公司大門（自動門 2 格寬），房間之間一律單格牆
        "#...........#",   // 門外大廳：掛畫掛在上面這道牆上（掛牆的不佔地板格）
        "#.TT.TT######",   // 圓椅、書架；右側是自助角的牆
        "#...........#",   // 自助角的空地（人站這裡買）——x7 必須通，否則北是牆、南是機器、
                           // 西是牆，這排就被封成孤島，vending_1/cooler_2 永遠走不到
        "#.TT.TT#TTTT#",   // 等候椅；自助角：販賣機、飲水機、影印機（排【一排】——
                           // 機器都是 2 格高，排兩排會上下疊在一起）
        "#...........#",
        "#...........#",
        "#T..T......T#",   // 盆栽
        "#...........#",
    ...
    static readonly (string name, int cx, int cy)[] WestWaypoints =
    {
        ...
        ("entrance@face_up", 6, 8),       // 大門【外】側：進出的人站這裡，也是開門的觸發點
        ("lobby_1", 4, 12), ("lobby_2", 7, 12),      // 訪客等候區
        ("vending_1", 9, 10), ("cooler_2", 11, 10),   // 販賣機／飲水機前
    };
